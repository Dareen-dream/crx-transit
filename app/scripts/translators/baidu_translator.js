...
class BaiduTranslator {
  ...
  requestText (text, callback) {
    ...
    window.fetch(API_URL, {
      ...
      .then(result => callback(this.formatResult(result)))
      .catch(() => callback(null))
  }
  ...
  formatResult (result) {
    ...
    if (_.has(result, 'dict.symbols[0].parts')) {
      ...
      const transResult = _.get(result, 'trans[0]', {})
      if (transResult.src === transResult.dst) {
        return null
      } else {
        response.translation = transResult.dst
      }
    }
...
app.registerTranslator(new BaiduTranslator())
